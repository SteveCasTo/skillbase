...
- Sileo muestra los avisos transitorios en la aplicación privada con la posición superior derecha y tema sincronizado con el tema activo. En `/app`, los colores de sus estados usan tokens semánticos de la interfaz privada.
...
- El formulario de cursos combina selectores accesibles de nivel/formato, controles separados de fecha civil y hora, calendario, horario por días/horas y toolbar Markdown para selección de texto. Con JavaScript, «Crear borrador» se habilita solo cuando los campos obligatorios están presentes y válidos, la nota mínima está en rango, las fechas civiles están en orden y las fechas opcionales de inscripción son ambas válidas o ambas vacías. Sin JavaScript, la página proporciona selects HTML para nivel y formato como alternativa progresiva. La fecha se introduce como DD/MM/AAAA y la hora civil boliviana en un control separado; el horario se persiste como cadena informativa. Los filtros de entrada ayudan a rechazar caracteres/ediciones inválidas sin reemplazar la validación autoritativa del servidor.
